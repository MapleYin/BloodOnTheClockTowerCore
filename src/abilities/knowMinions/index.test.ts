...
describe("KnowMinions", () => {
    ...
    it("shows in first night", () => {
        ...
        expect(firstTimeline.operations.map(o => o.abilityKey).includes("KnowMinions")).toBeTruthy();
    });

    it("does not show in second night", () => {
        game.nextTimeline(); // First day
        const secondNightTimeline = game.nextTimeline(); // Second night
        expect(secondNightTimeline.operations.map(o => o.abilityKey).includes("KnowMinions")).toBeFalsy();
    });
});